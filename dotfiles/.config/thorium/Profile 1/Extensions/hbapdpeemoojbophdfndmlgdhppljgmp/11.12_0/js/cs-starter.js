var Starter = (function(){
  ...
  var checkURL = function( sources, plan, config ){
    ...
    if (sources[source]) {
      ...
      if (hasCredits) {
        ...
        // config.areSubsEnabled = true;
        if (config.areSubsEnabled) freeSources = 'instgr openai pntrst youtub bingco duckgo';
  ...
  var isGSearchURL = function(){
    ...
    if (url.match(/google\.[\w.]+\/?$/)) return true;
    if (url.match(/google\.[\w.]+\/[\w=?&]+$/)) return true;
