var App = (function(){
  ...
  var initMessaging = function(){
    chrome.runtime.onMessage.addListener(
      function(request, sender, sendResponse) {
        ...
        else if (cmd === 'api.getPASFData') {
          if (!appEnabled) return;
          API.getPASFData( data, function( json ){
            sendResponse(json);
          });
          return true;
        }
  ...
  var initContextMenu = function(){
    ...
    chrome.contextMenus.create({
        ...
        //   chrome.tabs.create({
        //     url: 'https://keywordseverywhere.com/ke/2/analyze.php?id=' + encodeURIComponent(id)
        //   });
        // }
      });
    ...
    chrome.contextMenus.onClicked.addListener(function(onClickData, tab){
      ...
      if (onClickData.menuItemId.indexOf("ctxtGetKeywordData") !== -1) {
        ...
        urlsToAnalyze[id] = tab.url;
        chrome.tabs.create({
          url: chrome.runtime.getURL('/html/page.html?page=analyze&id=' + encodeURIComponent(id))
        });
      }
    });
  };
